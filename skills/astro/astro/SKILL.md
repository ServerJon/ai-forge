...
**Always use the Astro Docs MCP first for code examples and latest API.**

Use `mcp__astro-docs__search_astro_docs` to query the official Astro documentation directly. Only fall back to [docs.astro.build](https://docs.astro.build) if the MCP is unavailable or returns insufficient results.

```txt
# MCP tool — use this first
mcp__astro-docs__search_astro_docs(query: "<your topic>")
```
...
- **MCP (preferred)**: `mcp__astro-docs__search_astro_docs` — official docs via MCP
- [Docs](https://docs.astro.build) — fallback if MCP unavailable
