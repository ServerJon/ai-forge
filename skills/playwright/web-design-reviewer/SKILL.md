...
#### Checking Available Playwright Integration

Before setting up, check which Playwright integration is available:

1. **Playwright CLI** (preferred when available)
   - Check if installed: `npx playwright --version`
   - If available, use CLI commands directly for navigation, screenshots, and DOM inspection

2. **Playwright MCP** (fallback)
   - Use when CLI is not installed or when MCP server integration is preferred
   - Requires MCP server configuration (see below)

#### Playwright CLI (Optional)

When the CLI is available, use it directly via bash:

| Capability | CLI Command Example                                                    | Purpose                       |
| ---------- | ---------------------------------------------------------------------- | ----------------------------- |
| Screenshot | `npx playwright screenshot <url> <output.png>`                         | Capture full-page screenshots |
| PDF        | `npx playwright pdf <url> <output.pdf>`                                | Export page as PDF            |
| Script     | `npx playwright codegen <url>`                                         | Generate interaction scripts  |
| Responsive | `npx playwright screenshot --viewport-size=375,812 <url> <mobile.png>` | Mobile viewport capture       |

> **Note**: The CLI is best suited for snapshot-style tasks. For interactive flows (click, form fill, navigation), prefer a script or the MCP.

#### Playwright MCP (Fallback)

[Playwright MCP](https://github.com/microsoft/playwright-mcp) is the recommended fallback when the CLI is not available.
